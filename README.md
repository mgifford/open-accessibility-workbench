...
- 🟡 **Optional In-Browser AI with Validation Loop**: The consent gate, untrusted-data-safe response processing, invention checks, and bounded validation loop are all built, behind a **capability-driven provider abstraction** ([ADR 0001](docs/decisions/0001-capability-driven-browser-ai.md)) that routes across AI backends by probed capability — **never** browser brand: the browser's built-in **Prompt API** (Gemini Nano; no dependency, no build flag) when exposed, a build-gated **transformers.js** on-device runtime (download/cancel/dispose, WebGPU/WASM, Hugging Face or GitHub-release weights) when built and WebGPU is usable, and an always-available **deterministic** floor. Every route's output is shown as a clearly-labelled draft behind the same validation gate. The transformers.js route is **build-gated** (`VITE_AI_RUNTIME=1`) and **OFF in the default build**, which downloads no model from us and tree-shakes the ~100 MB dependency out; it stays scaffolded until verified on WebGPU hardware — see the [roadmap](docs/ROADMAP.md). Inference is always on-device; your report is never uploaded.
