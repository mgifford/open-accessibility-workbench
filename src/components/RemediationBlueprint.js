  ...
  render() {
    ...
    this.innerHTML = `
      <div class="card">
        ...
        <p style="font-size: var(--font-size-sm); color: var(--color-text-secondary); margin-bottom: var(--space-3);">${escapeHtml(bp.whatNeedsToChange)}</p>
        ...
        ${bp.humanDecisionsRequired?.length ? `
          <div style="background-color: var(--color-urgency-medium-bg); padding: var(--space-3); border-radius: var(--radius-sm); margin-bottom: var(--space-3);">
            ...
            <ul style="font-size: var(--font-size-xs); margin-left: var(--space-4); margin-top: var(--space-1);">${bp.humanDecisionsRequired.map(d => `<li>${escapeHtml(d)}</li>`).join('')}</ul>
          </div>
        ` : ''}
      </div>
    `;
