...
// Per-chunk provenance defaults (all chunks below are W3C-sourced techniques).
const CHUNK_REVISION = 'WCAG 2.2 (2023-10-05)';
...
// --- Provenance & licence enforcement (spec §10.2) --------------------------
// The build FAILS on missing source/licence/id, duplicate ids, an unknown corpus
// revision, or a licence outside the compatible allowlist.
const enforcedChunks = validateCorpus(guidanceChunks, {
  corpusRevision: sourceCorpusRevision,
  chunkRevision: CHUNK_REVISION,
  retrievedDate: RETRIEVED_DATE
});
manifest.itemCount = enforcedChunks.length;

fs.writeFileSync(path.join(ragDir, 'manifest.json'), JSON.stringify(manifest, null, 2));
fs.writeFileSync(path.join(ragDir, 'guidance.json'), JSON.stringify(enforcedChunks, null, 2));

// Emit an importable JS module so browser + Node consume the corpus without a
// JSON import attribute.
const generated =
  '// GENERATED by scripts/build-rag-index.js — run `npm run build:data`. Do not edit by hand.\n' +
  `export const RAG_MANIFEST = ${JSON.stringify(manifest, null, 2)};\n\n` +
  `export const RAG_CORPUS = ${JSON.stringify(enforcedChunks, null, 2)};\n`;
fs.writeFileSync(path.resolve(__dirname, '../src/guidance/rag-corpus.generated.js'), generated);

console.log(`RAG index generated: ${enforcedChunks.length} chunks, corpus rev ${sourceCorpusRevision}.`);
